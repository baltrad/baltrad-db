...
class TestApplication(object):
    ...
    def test_from_conf(self):
        ...
        with mock.patch("baltrad.bdbserver.web.app.Application") as ctor:
            ...
            assert(True == result.enable_remove_all_files)
    ...
    def test_from_conf_defaults(self):
        ...
        with mock.patch("baltrad.bdbserver.web.app.Application") as ctor:
            ...
            result = from_conf(self.backend, conf)
            ctor.assert_called_once_with(self.backend)
            assert(False == result.enable_remove_all_files)
...
def test_from_conf(backend_ctor, app_ctor, authmw_ctor):
    ...
    assert(mock.sentinel.authmw == app.from_conf(mock.sentinel.conf))

    assert(called_once_with(backend_ctor, mock.sentinel.conf))
    assert(called_once_with(mock_backend.is_operational))
    assert(called_once_with(app_ctor, mock_backend, mock.sentinel.conf))
    assert(called_once_with(authmw_ctor, mock.sentinel.app, mock.sentinel.conf))
