...
public class UnaryOperatorTest extends EasyMockSupport {
  UnaryOperator classUnderTestMock;
  UnaryOperator classUnderTest;

  @Before
  public void setUp() {
    classUnderTestMock = createMock(UnaryOperator.class);
    classUnderTest = new UnaryOperator() {
      @Override
      protected Expression executeUnary(Expression expr) {
        return classUnderTestMock.executeUnary(expr);
      }
    };
    resetAll();
  }
  ...
  public void execute() {
    ...
    expect(classUnderTestMock.executeUnary(expr))
      .andReturn(expected);
    replayAll();
