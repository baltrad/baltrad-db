...
TEST(oh5_Oh5Scalar_test, int64_) {
    ...
    EXPECT_THROW(v.double_(), std::invalid_argument);
}
...
TEST(oh5_Oh5Scalar_test, to_int64) {
    ...
    EXPECT_THROW(Oh5Scalar("foo").to_int64(), std::invalid_argument);
    EXPECT_EQ(10, Oh5Scalar(10).to_int64());
...
TEST(oh5_Oh5Scalar_test, to_double) {
    ...
    EXPECT_THROW(Oh5Scalar("foo").to_double(), std::invalid_argument);
    EXPECT_EQ(10.0, Oh5Scalar(10).to_double());
    EXPECT_EQ(1.2, Oh5Scalar(1.2).to_double());
}

TEST(oh5_Oh5Scalar_test, to_bool) {
    EXPECT_THROW(Oh5Scalar("foo").to_bool(), std::invalid_argument);
    EXPECT_THROW(Oh5Scalar("").to_bool(), std::invalid_argument);
...
TEST(oh5_Oh5Scalar_test, to_time) {
    EXPECT_THROW(Oh5Scalar("foo").to_time(), std::invalid_argument);
    EXPECT_EQ(Time(12, 34, 56), Oh5Scalar("123456").to_time());
    EXPECT_THROW(Oh5Scalar(10).to_time(), std::invalid_argument);
    EXPECT_THROW(Oh5Scalar(1.2).to_time(), std::invalid_argument);
}

TEST(oh5_Oh5Scalar_test, to_date) {
    EXPECT_THROW(Oh5Scalar("foo").to_date(), std::invalid_argument);
    EXPECT_EQ(Date(2000, 11, 12), Oh5Scalar("20001112").to_date());
    EXPECT_THROW(Oh5Scalar(10).to_date(), std::invalid_argument);
    EXPECT_THROW(Oh5Scalar(1.2).to_date(), std::invalid_argument);
}
