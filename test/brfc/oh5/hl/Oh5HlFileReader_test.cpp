...
class oh5_hl_Oh5HlFileReader_test : public ::testing::Test {
  public:
    oh5_hl_Oh5HlFileReader_test()
            : reader()
            , file() {
    }
...
TEST_F(oh5_hl_Oh5HlFileReader_test, test_read_nxfile) {
    EXPECT_THROW(reader.read("/path/to/nxfile", file), fs_error);
}

TEST_F(oh5_hl_Oh5HlFileReader_test, test_load_from_filesystem) {
